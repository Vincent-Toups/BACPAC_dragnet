...
qs = pl.concat([(pl.read_csv(file, try_parse_dates=True).select(columns)
                 .with_columns(c('VISITNUM').cast(pl.Int64).alias('VISITNUM'))
                 .with_columns(c('QSSTRESN')
                               .cast(str)
                               .map_elements(handle_qsstresn_conversion)
                               .alias('QSSTRESN')))
                 for file in meta_data["file"]]);

qs.write_csv("derived_data/qs-collected.csv")

study_counts = qs.group_by('STUDYID').count().with_columns(c('count').alias('Row Count')).drop('count');
subject_counts = (qs
...
pain_interference = pain_interference.filter(c('Visit Number').is_not_null()).unique();
...
sorted_visit_numbers = [str(vn) for vn in list(pain_interference.group_by('Visit Number').count().sort('Visit Number')['Visit Number'])];
pain_int_with_nulls = (pain_interference
                       .drop('Visit Date')
                       .unique()
                       .pivot(index='USUBJID',values='Pain Interference',on='Visit Number',aggregate_function=pl.element().mean())
                       .select(['USUBJID']+sorted_visit_numbers)
                       .unpivot(index='USUBJID',variable_name='Visit Number',value_name='Pain Interference')
...
pile = [(df[1]
     ...
     .with_columns(c('Pain Interference')
                   .ewm_mean(alpha=0.4)
                   .alias('Pain Interference (Smoothed)'))) for df in pain_int_with_nulls.group_by('USUBJID')];
...
(pain_interference
 ...
 .group_by('Samples').count().sort('Samples'))

pi_initial = pain_interference.group_by('USUBJID')
